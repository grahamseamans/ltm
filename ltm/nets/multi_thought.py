...
class MemNet(nn.Module):
    def __init__(
        ...
        self.attn_1 = nn.MultiheadAttention(
            embed_dim=embed_dim,
            num_heads=num_heads,
            kdim=self.memory.mem_len,
            vdim=self.memory.mem_len,
            batch_first=True,
        ).to(device())
    ...
    def forward(self, obs, state=None):
        ...
        sim_mems = self.topk(obs_mem, obs, self.k)
    ...
    def topk(self, mems, obs, k):
        ...
        num_mems, mem_dim = mems.shape
        obs = torch.unsqueeze(obs, dim=1)
        mems = mems.expand(batch_size, num_mems, mem_dim)
